...
$("#return").click(function () {
  ...
  $(window).width() <= 750 &&
    $("#flightSearch").attr("action", "/M_Roundtrip_Search.bc");
  $("#flightSearch").show();
  document.querySelector("#inp2-flight").classList.remove("Noactive-date");
});
...
$("#direct").click(function () {
  $(this).addClass("active-r-btn"),
    ...
    $(window).width() <= 750 &&
      $("#flightSearch").attr("action", "/M_Oneway_Search.bc"),
    $("#flightSearch").show();
  document.querySelector("#inp2-flight").classList.add("Noactive-date");
});
    ...
    if (($(t).closest("ul").next().val(n), e < n)) {
      ...
      for ($(t).closest("form").find(".countRoom").empty(), i = 1; i <= s; i++)
        $(t)
          ...
          .find(".countRoom")
          .append(
            '<div class="contentRooms contentRoom"><div class="numberOfRooms RoomRow  text-sm text-primary float-left clear-both w-full mb-1 text-left">room' +
              i +
              '</div><div class="itemlable2 w-full mb-2 flex flex-col float-right relative "><label class="float-left text-sm leading-[48px]" for="textbox-adultcount' +
              i +
              '"> <span class="notshow">adult</span></label><ul class="button-click float-right w-full h-12 leading-12 rounded-[7px]"><li class="float-right hover:shadow-lg w-12 h-12 leading-12 text-center rounded-full bg-primary-50 p-3 flex justify-center items-center"><div class="button secondary h-full w-full flex rounded-full border  border-secondary-500 !text-lg !text-secondary-500 justify-center items-center leading-[48px]" onclick="increaseAdult(this)"><span class="hide">+</span></div></li><li class="float-left hover:shadow-lg w-12 h-12 leading-12 text-center rounded-full bg-primary-50 p-3 flex justify-center items-center"><div class="button secondary h-full w-full flex rounded-full border  border-secondary-500 !text-lg !text-secondary-500 justify-center items-center leading-[48px]"  onclick="decreaseAdult(this)"><span class="hide">-</span></div></li></ul><input type="text" class="cat_textbox  adultcount adult-count  bg-secondary-500  rounded-full text-white w-12 h-12 leading-12 absolute right-[40%] top-[50%] text-center" id="textbox-adultcount' +
              i +
              '" name="_roo$(t).rooms__' +
              i +
              '.adultcount" maxlength="4000" value="2"> <div class="clr"></div></div><div class="itemlable2 w-full mb-2 flex flex-col float-right relative "> <label class="float-left text-sm leading-[48px]" for="textbox-childcount' +
              i +
              '"> <span class="notshow">child</span> </label><ul class="button-click-childRoom  w-full h-12 leading-12 rounded-[7px]"><li class=" float-right hover:shadow-lg w-12 h-12 leading-12 text-center rounded-full bg-primary-50 p-3 flex justify-center items-center"><div class="button secondary h-full w-full flex rounded-full border  border-secondary-500 !text-lg !text-secondary-500 justify-center items-center leading-[48px]" onclick="increaseChild(this)"><span class="hide">+</span></div></li><li class="float-left hover:shadow-lg w-12 h-12 leading-12 text-center rounded-full bg-primary-50 p-3 flex justify-center items-center"><div class="button secondary h-full w-full flex rounded-full border  border-secondary-500 !text-lg !text-secondary-500 justify-center items-center leading-[48px]" onclick="decreaseChild(this)"><span class="hide">-</span></div></li><div class="clr"></div></ul><input type="text" class="cat_textbox  chcount child-count bg-secondary-500  rounded-full text-white w-12 h-12 leading-12 absolute right-[40%] top-[50%] text-center" id="textbox-childcount' +
              i +
...
function increaseAdult(t) {
  var e = $(t),
    ...
    n = e.text().indexOf("+") > -1 ? i + 1 : i > 0 ? i - 1 : 0;
...
function increaseChild(t) {
  var e = $(t),
    ...
    o = e.text().indexOf("+") > -1 ? n + 1 : n > 0 ? n - 1 : 0,
    s = "";
...
$(".button-click .button").on("click", function () {
  var t = $(this),
    e = parseInt(t.closest("ul").next().val()),
    i = t.text().indexOf("+") > -1 ? e + 1 : e > 0 ? e - 1 : 0;
...
var createChildDropdown = function (t) {
    var e = $("<div />", {
      class:
        "createChildDropdown IRANSansWeb mb-4 w-full float-right clear-both",
    });
    return (
      e.append(
        $("<label />", {
      ...
      e.append(
        $(
          '<select class="select-age w-full float-left bg-[#F8F8F8]   h-12 leading-12 rounded-[7px] px-2" id="select-age' +
            t +
            '"/>'
        )
      ),
...
function addMulticityRoute(t) {
  if (
    ...
    (i.innerHTML = e),
      $(window).width() >= 1024
        ? (i.className =
            "route-content set_Date_Box float-left w-full relative mb-3")
        : (i.className = "route-content float-left relative w-full"),
      ...
      i.insertAdjacentHTML(
        "beforeend",
        '<div class="route-minus-btn cursor-pointer clear-both absolute top-0 right-2 bg-third rounded-full max-xl:-right-[10px] max-xl:bg-inherit" onclick="deleteMulticityRoute(this)"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="red" class="w-6 h-6">\n  <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />\n</svg>\n</div>'
      ),
...
document.querySelectorAll(".hotel-input").forEach((pass) => {
  pass.addEventListener("click", () => {
    document.querySelectorAll(".searchList").forEach((searchList) => {
